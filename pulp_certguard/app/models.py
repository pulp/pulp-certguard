...
cert_unquoted_body_regex = re.compile(
...
class BaseCertGuard(ContentGuard):
    ...
    def _get_client_cert_header(cls, request):
        try:
            ...
            msg = _(
                "A client certificate was not received via the `X-CLIENT-CERT` header."
            )
            logger.warning(msg)
    ...
    def _ensure_client_cert_is_trusted(self, unquoted_certificate):
        ...
        try:
            ...
            if (
                exc.args[0][0] == 20
            ):  # The error code for client cert not signed by the CA
                msg = _(
                    "Client certificate is not signed by the stored 'ca_certificate'."
                )
...
class RHSMCertGuard(BaseCertGuard):
    ...
    def _check_paths(rhsm_cert, path):
        ...
        if rhsm_cert.check_path(path) is False:
            ...
            msg = _(
                "Requested path is not a subpath of a path in the client certificate."
            )
            raise PermissionError(msg)
