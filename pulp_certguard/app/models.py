...
class CertGuard(ContentGuard):
    ...
    def permit(self, request):
        ...
        Raises:
            HTTPForbidden: When the request cannot be authorized.
        """
...
class Validator:
    ...
    def client_certificate(self, request):
        ...
        try:
            ...
            reason = _(
                'HTTP header "{h}" not found for: {u}.').format(
                    h=name,
                    u=request.url)
            log.debug(reason)
    ...
    def __call__(self, request):
        ...
        Raises:
            HTTPForbidden: When validation the client certificate
                cannot be validated.
        """
        try:
            ...
            log.debug(_('Client certificate cannot be validated.'))
            raise HTTPForbidden()
        except Exception as pe:
            log.debug(str(pe), exc_info=True)
            raise HTTPForbidden()
