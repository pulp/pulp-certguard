...
then
  ...
  docker exec pulp pip3 install pulp-certguard-client==$VERSION
  mkdir -p dist
...
find . -name "*.whl" -exec docker exec pulp pip3 install /root/pulp-openapi-generator/pulp_certguard-client/{} \;
...
tar cvf ../../pulp-certguard/python-client-docs.tar ./docs
exit $?
