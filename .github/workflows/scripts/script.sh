...
if [ -f $FUNC_TEST_SCRIPT ]; then
  source $FUNC_TEST_SCRIPT
    ...
    if [[ "$GITHUB_WORKFLOW" == "Certguard Nightly CI/CD" ]]; then
        pytest -v -r sx --color=yes --suppress-no-test-exit-code --pyargs pulp_certguard.tests.functional -m parallel -n 8  --nightly
        pytest -v -r sx --color=yes --pyargs pulp_certguard.tests.functional -m "not parallel"  --nightly
